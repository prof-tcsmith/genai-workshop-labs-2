...
st.success(
    "**You've now built every block: prompt → memory → guardrails → grounding → RAG → tools/agents → governance → red-team.** "
    "Next, see them assembled into a real application — "
    "**[Course Content Studio ↗](https://genai-workshop-labs-awybgq8gnmnrevxna2ukv3.streamlit.app/)** — "
    "which *uses and extends* these blocks (real vector DB, real database, real MCP service) to turn your course materials "
    "into a **Canvas QTI quiz**."
)
