"""Local in-memory vector store over the seeded coverage policies.

The local-backed replacement for a cloud vector DB (Pinecone): it embeds the
policy chunks with OpenAI and ranks them by cosine similarity in numpy. Built
...
_POLICIES = os.path.join(os.path.dirname(__file__), "..", "seed", "policies")
...
def _build() -> None:
    global _index
    chunks = []
    for path in sorted(glob.glob(os.path.join(_POLICIES, "*.md"))):
